 ...
 */
public class CBPageAdapter<T extends BannerData> extends RecyclerView.Adapter<Holder> {
    ...
    public Holder onCreateViewHolder(ViewGroup parent, int viewType) {
        int layoutId = creator.getLayoutId(viewType);
        View itemView = LayoutInflater.from(parent.getContext()).inflate(layoutId, parent, false);
        helper.onCreateViewHolder(parent, itemView);
        return creator.createHolder(itemView, viewType);
    }
    ...
    public int getItemViewType(int position) {
        if (datas != null) {
            int realPosition = position % datas.size();
            return datas.get(realPosition).getType();
        } else {
            return 0;
        }
    }
