 ...
 * @author Sai 支持自动翻页
 */
public class ConvenientBanner<T extends BannerData> extends RelativeLayout {
    ...
    /**
     * 设置当前页对应的position，使用data的type进行判断，position是这个type下的第几个
     */
    public ConvenientBanner setCurrentItemWithType(int type, int position, boolean smoothScroll) {
        int realPosition = 0;
        for (int i = 0; i < mDatas.size(); i++) {
            BannerData bannerData = mDatas.get(i);
            if (bannerData.getType() == type) {
                if (position == 0) {
                    cbLoopScaleHelper.setCurrentItem(canLoop ? mDatas.size() + realPosition : realPosition, smoothScroll);
                    break;
                } else {
                    position--;
                }
